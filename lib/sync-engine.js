...
function getFilePath(settings) {
  return getDirectoryPath(settings) + '/bookmarks.json';
}

function getTimestampedBackupPath(settings) {
  const now = new Date();
  ...
  return getDirectoryPath(settings) + `/bookmarks-backup-${stamp}.json`;
}

function getDirectoryPath(settings) {
...
export async function oneWayBackup() {
  return withLock(async () => {
    ...
    // 先保存带时间戳的历史备份，再更新固定文件名作为最新快照。
    await client.put(getTimestampedBackupPath(settings), localTree);
